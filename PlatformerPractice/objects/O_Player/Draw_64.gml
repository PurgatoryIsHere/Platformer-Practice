/// @description Diplay Health and Lives
// You can write your code in this editor

draw_set_font(global.menu_options_font);
draw_set_halign(global.default_halign);
draw_set_valign(global.default_valign);

if(global.lives >= 0 && !justDied)
{
	// Life counter
	draw_sprite_ext(S_PlayerLives, 0, 25, 25, 2, 2, 0, c_white, 1);
	draw_text_outline(50, 17.5, "x" + string(global.lives), .7, .7, c_white, c_black);

	// Health Display
	draw_sprite_ext(S_HealthRestore, 0, 10, 50, 2, 2, 0, c_white, 1);
	draw_text_outline(50, 52.5, string(global.player_health), .7, .7, c_white, c_black);
}

if(fade_alpha > 0) 
{
	draw_set_alpha(fade_alpha);
	draw_set_color(c_black);
	draw_rectangle(0, 0, display_get_width(), display_get_height(), false);
	draw_set_alpha(1);
}